...
function createIncomeMessageItemLi (message, time, avatarUrl) {
  ...
  li.innerHTML = `<div class="message-data">
                        ...
                        <span class="message-data-time" data-timestamp="${time}">${formatTimestamp(
    time
  )}</span>
                    </div>
...
function createChatHeader (data) {
  ...
  div.innerHTML = `<img src="${localStorage.getItem(
    'companionPicture'
  )}" alt="avatar">
...
function connectToSocket () {
  chatSocket = new WebSocket(
    `ws://${window.location.host}/ws/chat/${chatToken}/`
  )
