...
function loadFriendsPage () {
  ...
  document
    .getElementById('friendRequestInput')
    .addEventListener('input', searchFriendQuery)
  loadFriendsList()
  ...
  fetchWithToken(url, {
    ...
    .then(data => {
      if (data.data.length === 0) {
        ...
        noFriendsRequests.setAttribute(
          'class',
        ...
        friendRequestTable.setAttribute(
          'class',
          'table table-striped table-hover table-bordered'
        )
        ...
        data.data.forEach(function (friendRequest) {
          ...
          friendRequestDiv.innerHTML = `
                <th scope="row">${
                  i + page * FRIEND_REQUESTS_LIST_PAGE_SIZE
                }</th>
                <td><span class="d-inline-block text-truncate" style="max-width: 150px;"
                data-bs-toggle="tooltip" title="${friendRequest.email}">${
            friendRequest.email
          }</span></td>
                <td><span class="d-inline-block text-truncate" style="max-width: 150px;"
                data-bs-toggle="tooltip" title="${friendRequest.username}">${
        ...
        friendRequestTableBody
          .querySelectorAll('#accept-friend-request')
          .forEach(button => {
            button.addEventListener('click', acceptFriendRequest)
          })
        friendRequestTableBody
          .querySelectorAll('#delete-friend-request')
          .forEach(button => {
      ...
      if (data.data.length === 0) {
        ...
        data.data.forEach(function (friend) {
          ...
          friendDiv.innerHTML = `
                    ...
                    <td><span class="d-inline-block text-truncate" style="max-width: 150px;"
                        data-bs-toggle="tooltip" title="${friend.email}">${
            friend.email
          }</span></td>
                    <td><span class="d-inline-block text-truncate" style="max-width: 150px;"
                        data-bs-toggle="tooltip" title="${friend.username}">${
            friend.username
          }</span></td>
                    <td>
