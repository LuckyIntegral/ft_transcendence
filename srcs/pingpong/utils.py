...
def sendVerificationEmail(email, token):
    try:
        send_mail(
            "Email Verification",
            "Verify your email address by clicking the link below: https://10.14.5.2:8443/verify-email?token=" + token,
            "admin@localhost",
            [email],
            fail_silently=False,
        )
...
def sendPasswordResetEmail(email, token):
    try:
        send_mail(
            "Password Reset",
            "Reset your password by clicking the link below: https://10.14.5.2:8443/reset-password/?token=" + token,
            "admin@localhost",
            [email],
            fail_silently=False,
        )
