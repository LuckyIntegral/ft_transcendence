...
class PingPongConsumer(AsyncWebsocketConsumer):
    ...
    def createMessageRecipient(self, recipient):
        if PingPongConsumer.userCounter == 2:
            return MessagesRecipient.objects.create(
                recipient=recipient, isRead=True
            )
    ...
    async def receive(self, text_data):
        ...
        self.message = await self.createMessage(
            self.user, messageText, self.messageRecipient
        )
...
class GameLobbyConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        await self.channel_layer.group_send(
            self.game_id, {"type": message_type, "message": message}
        )
...
class UserConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        await self.channel_layer.group_send(
            self.user_id, {"type": "chat_message", "message": message}
        )
